#include "../../../include/components/capture/rtspthread.h"
#include <QDebug>
        ...
        if (pkt->stream_index == video_stream_index) {
            ...
            if (avcodec_send_packet(codec_ctx, pkt) == 0) {
                while (avcodec_receive_frame(codec_ctx, frame) == 0) {
                    qDebug() << "Decoded frame:" << frame->width << "x" << frame->height;
                    qDebug() << "Frame format:" << av_get_pix_fmt_name((AVPixelFormat)frame->format);

                    if (frame->format != AV_PIX_FMT_YUV420P && frame->format != AV_PIX_FMT_YUVJ420P && frame->format != AV_PIX_FMT_NV12) {
                        qDebug() << "Unexpected pixel format:" << frame->format;
                        continue;
                    }
